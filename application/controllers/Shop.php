...
class Shop extends CI_Controller
{
    ...
    public function checkout($action = '')
    {
        ...
        if (! is_login()) {
            ...
            // verifikasi apakah ada sesi login 'customer' jika tidak maka disuruh login
        ...
        switch ($action) {
            default:
                ...
                // untuk mengecek apakah memakai kupon atau tidak
                if (empty($coupon)) {
                    // jika tidak memakai kupon
                    $discount = 0;
                    $disc = 'Tidak menggunkan kupon';
                } else {
                    // jika memakai kupon kemudian cek apakah kupon cocok atau tidak
                    if ($this->customer->is_coupon_exist($coupon)) {
                        // jika kupon cocok kemudian cek status kupon
                        if ($this->customer->is_coupon_active($coupon)) {
                            // jika kupon statusnya sudah mati karena kadarluarsa
                            if ($this->customer->is_coupon_expired($coupon)) {
                                $discount = 0;
                                $disc = 'Kupon kadaluarsa';
                            } else {
                                // jika kupon statusnya masih aktif dan set kupon promo
                            ...
                            // jika kupon cocok tapi statusnya sudah tidak aktif
                            $discount = 0;
                            $disc = 'Kupon sudah tidak aktif';
                        }
                    } else {

                        // jika kupon tidak terdaftar
                        $discount = 0;
    ...
    // fungsi untuk menbuat id transaksi otomatis
